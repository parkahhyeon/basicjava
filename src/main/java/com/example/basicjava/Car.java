...
/* 필드 (field) : 클래스에 포함된 변수(variable)를 의미함.
    ...
    * 필드의 초기화
        ...
        * 하지만 지역 변수와 마찬가지로 적절한 값으로 초기화한 후에 사용하는 것이 좋음.
        1. 명시적 초기화 : 지역 변수를 초기화하는 방법과 마찬가지로 필드를 선언과 동시에 초기화하는 방법.
        2. 생성자를 이용한 초기화 : 객체의 생성과 동시에 필드를 초기화 하는 방법. 따라서 인스턴스를 생성할 때까지 필드 초기화 못 함.
        3. 초기화 블록을 이용한 초기화
            * 초기화 블록 (initialization block) : 클래스 필드의 초기화만을 담당하는 중괄호({})로 둘러싸인 블록을 의미.
                * 초기화 블록은 생성자보다 먼저 호출되며, static 키워드의 유무에 따라 다음과 같이 구분함.
                * 초기화 블록에는 다양한 명령문 및 제어문을 사용할 수 있으므로, 복잡한 초기화를 해야 할 경우 사용함.
                (1) 인스턴스 초기화 블록 : 인스턴스변수의 복잡한 초기화에 사용
                (2) 클래스 초기화 블록 : 클래스변수의 복잡한 초기화에 사용
                * 인스턴스 초기화 블록
                    * 단순히 클래스내에 블록({중괄호})을 만들고 그 안에 코드를 작성해 사용함.
                    * 생성자와 같이 인스턴스를 생성할 때 마다 수행됨.
                    * 언제나 생성자보다 인스턴스 초기화 블록이 먼저 실행됨.
                    * 생성자와 인스턴스 초기화 블로그이 차이는 거의 없으므로 인스턴스 초기화 블록은 잘 사용되지 않음.
                    * 인스턴스 변수의 초기화는 주로 생성자를 사용하고,
                    * 모든 생성자에서 공통으로 수행돼야 하는 코드를 인스턴스 초기화 블록에 포함하여 코드의 중복을 제거함.
                * 클래스 초기화 블록
                    * 인스턴스 초기화 블록 앞에 단순히 static 키워드를 덧붙이기만 하면 됨.
                    * 클래스가 메모리에 처음 로딩될 때 단 1번만 수행죔.
                      (클래스가 처음 로딩될 때 클래스변수들이 자동적으로 메모리에 만들어지고, 곧바로 클래스 초기화 블럭이 클래스변수들을 초기화하게 되는 것)
                    * 생성자나 인스턴스 초기화 블록으로는 수행할 수 없는 클래스 변수의 초기화를 수행할 때 사용.

        * <필드의 초기화 시기와 순서>
            클래스 변수는 인스턴스 변수보다 항상 먼저 생성되고 초기화 됨.
            * 클래스변수의 초기화시점 : 클래스가 처음 로딩될 때 단 한 번 초기화된다.
            * 인스턴스변수의 초기화시점 : 인스턴스가 생성될 때마다 각 인스턴스별로 초기화됨.
            * 클래스변수의 초기화순서 : 기본값 -> 명시적초기화 -> 클래스 초기화 블록
            * 인스턴스변수의 초기화순서 : 기본값 -> 명시적초기화 -> 인스턴스 초기화 블록 -> 생성자
        * 같은 필드를 여러 번 초기화하면, 제일 마지막으로 초기화한 값만이 남게 됨.
* */
